...
interface GlobalOptions {
  ...
  token?: string
  insecure?: boolean
  /** path to a .env file, or false when --no-env-file was passed */
  envFile?: string | false
}

function getConnection(options: GlobalOptions): { url: string; token: string; insecure: boolean } {
...
async function withClient<T>(options: GlobalOptions, fn: (client: XoClient) => Promise<T>): Promise<T> {
...
const program = new Command()
  ...
  .hook('preAction', thisCommand => {
    const { envFile } = thisCommand.opts<GlobalOptions>()
...
program
  ...
  .action(async (cmdOptions: { output?: string }) => {
    try {
      await withClient(program.opts<GlobalOptions>(), async client => {
...
program
  ...
  .action(async (configPath: string) => {
    try {
      const exitCode = await withClient(program.opts<GlobalOptions>(), async client => {
...
program
  ...
  .action(async (configPath: string, cmdOptions: { prune?: boolean; yes?: boolean; dryRun?: boolean }) => {
    try {
...
program.action(async () => {
  try {
    ...
    const { envFile } = program.opts<GlobalOptions>()
    if (envFile !== false) {
      loadDotenv(typeof envFile === 'string' ? envFile : '.env')
    }
