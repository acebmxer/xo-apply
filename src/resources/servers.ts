import type { ServerSpec } from '../config/schema.js'
import type { XoServer } from '../client/index.js'
import type { FieldChange } from './remotes.js'

/** The XO-side fields this tool manages for a pool connection (server). */
...
export function serverSpecToDesired(spec: ServerSpec): DesiredServer {
  return {
    host: spec.host,
 ...
 */
export function serverToSpec(server: XoServer): ExportedServer {
 ...
 */
export function diffServer(desired: DesiredServer, actual: XoServer): FieldChange[] {
  const changes: FieldChange[] = []
  const normLabel = (v: string | undefined) => (v === undefined || v === '' ? undefined : v)
