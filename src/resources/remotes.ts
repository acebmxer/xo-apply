import { format, parse, type ParsedRemote } from 'xo-remote-parser'
import type { RemoteSpec } from '../config/schema.js'
import type { XoRemote } from '../client/index.js'
import { deepEqual } from './patterns.js'

/** The XO-side fields this tool manages for a remote. */
export interface DesiredRemote {
  name: string
  url: string
  /** mount options string (nfs), XO's separate `options` field */
  options?: string
  proxy?: string
}

export function remoteSpecToDesired(spec: RemoteSpec): DesiredRemote {
...
function extractUrlOptions(parsed: ParsedRemote): Record<string, string | number | boolean> | undefined {
...
export interface ExportedRemote {
  ...
  /** set when a secret was replaced by a ${env:...} placeholder */
  secretEnvVar?: string
}

export function envVarNameForRemote(remoteName: string): string {
...
export function remoteToSpec(remote: XoRemote): ExportedRemote {
  const parsed = parse(remote.url)
  const urlOptions = extractUrlOptions(parsed)
  const base: Record<string, unknown> = { name: remote.name }
  ...
  switch (parsed.type) {
    ...
    case 'file': {
      const spec: Record<string, unknown> = { ...base, type: 'local', path: parsed.path }
      if (urlOptions) {
        spec.urlOptions = urlOptions
      }
      return { spec }
...
export function diffRemote(desired: DesiredRemote, actual: XoRemote): FieldChange[] {
  ...
  const normOpt = (v: string | null | undefined) => (v == null || v === '' ? undefined : v)
  ...
  const normProxy = (v: string | null | undefined) => (v == null ? undefined : v)
