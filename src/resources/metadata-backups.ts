import type { MetadataBackupSpec, MetadataScheduleSpec } from '../config/schema.js'
import type { XoMetadataBackupJob, XoSchedule } from '../client/index.js'
import { deepEqual, extractIds, idPattern } from './patterns.js'
import type { FieldChange } from './remotes.js'
import { matchSchedules, type ScheduleChange } from './backup-jobs.js'

export interface DesiredMetadataSchedule {
  name: string
  cron: string
  enabled: boolean
  timezone?: string
...
export interface DesiredMetadataJob {
  name: string
  xoMetadata: boolean
  poolIds: string[]
  remoteNames: string[]
  settings: Record<string, unknown>
  schedules: DesiredMetadataSchedule[]
}

export function metadataSpecToDesired(spec: MetadataBackupSpec): DesiredMetadataJob {
  return {
    name: spec.name,
...
function metadataScheduleSpecToDesired(spec: MetadataScheduleSpec): DesiredMetadataSchedule {
  return {
    name: spec.name,
    cron: spec.cron,
...
export interface ActualMetadataJob {
  job: XoMetadataBackupJob
  schedules: XoSchedule[]
}

export interface MetadataJobDiff {
  changes: FieldChange[]
...
function actualRemoteNames(job: XoMetadataBackupJob, mapping: RemoteMapping): string[] {
 ...
 */
export function diffMetadataJob(
  desired: DesiredMetadataJob,
  actual: ActualMetadataJob,
...
function toGenericSchedule(s: DesiredMetadataSchedule): import('./backup-jobs.js').DesiredSchedule {
...
export function metadataScheduleSettings(s: DesiredMetadataSchedule): Record<string, unknown> {
  return {
    ...s.settings,
