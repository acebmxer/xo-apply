...
export async function fetchActualState(client: XoClient): Promise<ActualState> {
  const [remotes, jobs, metadataJobs, mirrorJobs, callJobs, schedules, vms, users, groups, servers] =
    await Promise.all([
      client.listRemotes(),
...
export async function applyPlan(client: XoClient, plan: Plan, options: ApplyOptions = {}): Promise<void> {
  ...
  for (const serverPlan of plan.servers) {
    ...
    if (serverPlan.kind === 'create') {
      ...
      if (desired.password === undefined) {
        throw new Error(
          `server "${desired.host}": cannot create without a password — add a \`password:\` (or \`password: \${env:...}\`) line and re-apply`
        )
      }
      ...
      const setBody: Parameters<typeof client.setServer>[0] = { id: serverPlan.actual.id }
      let needsSet = false
  ...
  for (const userPlan of plan.users) {
    ...
    if (userPlan.kind === 'create') {
      ...
      if (desired.password === undefined) {
        throw new Error(
          `user "${desired.email}": cannot create without a password — add a \`password:\` (or \`password: \${env:...}\`) line and re-apply`
        )
      }
      ...
      const body: { id: string; permission?: string } = { id: userPlan.actual.id }
  ...
  const resolveUserIds = (groupName: string, emails: string[]): string[] =>
    emails.map(email => {
