...
export function renderPlan(plan: Plan, { prune = false } = {}): string {
  const lines: string[] = []
  let creates = 0
  let updates = 0
  let deletes = 0
  ...
  if (plan.jobsManaged) {
    ...
    for (const j of plan.jobs) {
      if (j.kind === 'create') {
        ...
        lines.push(
          pc.green(`  + create  ${j.desired.name}`) +
  ...
  const renderSimpleJobs = <D extends { name: string }>(
    title: string,
    managed: boolean,
    jobs: Array<{ kind: string; desired: D; diff: { changes: { field: string; from: unknown; to: unknown }[] } }>,
    untrackedJobs: Array<{ name: string }>,
    detail: (d: D) => string
  ) => {
    if (!managed) return
  ...
  renderSimpleJobs(
    ...
    plan.metadataJobs,
    plan.untrackedMetadataJobs,
    d => (d.xoMetadata ? 'XO metadata' : 'pool metadata')
  )
  renderSimpleJobs(
    'Mirror backups',
    plan.mirrorManaged,
    plan.mirrorJobs,
    plan.untrackedMirrorJobs,
    d => `mirror ${d.mode}`
  )
  ...
  const renderChangeItems = <D extends { name?: string; email?: string }>(
    ...
    if (trailer) lines.push(pc.dim('  ' + trailer))
    lines.push('')
  }
  ...
  renderChangeItems(
    ...
    d => d.email,
    d => d.permission,
    plan.externalUserCount > 0 ? `${plan.externalUserCount} external user(s) unmanaged` : undefined
  )
  renderChangeItems(
    'Groups',
    plan.groupsManaged,
    plan.groups,
    plan.untrackedGroups,
    d => d.name,
