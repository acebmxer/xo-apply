...
export function exportSpec(actual) {
    ...
    // -- metadata backups -----------------------------------------------------
    const metadataBackups = [];
    for (const job of actual.metadataJobs) {
        const spec = { name: job.name };
        if (job.xoMetadata)
            spec.xoMetadata = true;
        ...
        spec.schedules = scheds.map((s, i) => {
            ...
            if (typeof set.retentionPoolMetadata === 'number')
                out.poolRetention = set.retentionPoolMetadata;
            if (typeof set.retentionXoMetadata === 'number')
                out.xoRetention = set.retentionXoMetadata;
            const extra = Object.fromEntries(Object.entries(set).filter(([k]) => k !== 'retentionPoolMetadata' && k !== 'retentionXoMetadata'));
            if (Object.keys(extra).length > 0)
                out.settings = extra;
            return out;
        });
    ...
    for (const job of actual.mirrorJobs) {
        const spec = { name: job.name, mode: job.mode };
        ...
        spec.schedules = scheds.map((s, i) => {
            ...
            if (s.timezone !== undefined)
                out.timezone = s.timezone;
            const set = job.settings[s.id] ?? {};
            if (typeof set.exportRetention === 'number' && set.exportRetention > 0)
                out.retention = set.exportRetention;
    ...
    for (const j of actual.jobs)
        jobNameById.set(j.id, j.name);
    for (const j of actual.metadataJobs)
        jobNameById.set(j.id, j.name);
    for (const j of actual.mirrorJobs)
        jobNameById.set(j.id, j.name);
    const stepByScheduleId = new Map();
    ...
    const triggerByJobId = new Map();
    for (const s of actual.schedules)
        triggerByJobId.set(s.jobId, s);
    const sequences = [];
    for (const job of actual.callJobs) {
        if (job.type !== 'call' || job.method !== SEQUENCE_METHOD)
            continue;
        ...
        const spec = { name: job.name, steps };
    ...
    if (metadataBackups.length > 0)
        doc.metadataBackups = metadataBackups;
    if (mirrorBackups.length > 0)
